...
| Mode | Command | 용도 |
|---|---|---|
| trigger | `windforce-lite trigger` | 외부 실행 요청 ingress |
| api | `windforce-lite api` | control plane, run 조회, HITL resume |
| worker only | `windforce-lite worker` | queue polling과 action 실행만 담당 |
| standalone | `windforce-lite standalone` | 개발/소규모용 trigger+api+worker 단일 프로세스 |
| sync | `windforce-lite sync` | request path 밖에서 catalog/bundle 갱신 |

Kubernetes production 배치는 다음 분리를 기본값으로 한다.

```text
windforce-lite trigger --state postgres
...
wire only the adapter routes they own.
