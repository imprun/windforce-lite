...
- 상태: Windforce Core scope complete ([#76](https://github.com/imprun/windforce-core/issues/76))
- 결정 문서: [ADR 0008](adr/0008-control-plane-event-webhooks.md)
- 구현 소유: `windforce-core` Control Plane
...
- 상태: Completed ([transactional store #69](https://github.com/imprun/windforce-core/issues/69))
...
- 상태: Completed ([#71](https://github.com/imprun/windforce-core/issues/71))
...
| Redirect | 비활성화 |
| User-Agent | `windforce-core-webhook/<VERSION>` |
...
- 상태: Completed ([#73](https://github.com/imprun/windforce-core/issues/73))
...
- 상태: Completed ([#74](https://github.com/imprun/windforce-core/issues/74))
...
- 상태: Completed ([#75](https://github.com/imprun/windforce-core/issues/75))
...
- 상태: Windforce Core contract completed ([#76](https://github.com/imprun/windforce-core/issues/76))

Messenger connector는 별도 소유 repository와 deployable service로 구현한다. Windforce Core는 다음 통합 자산만 소유한다.
...
별도 connector repository에서 첫 provider로 Slack Incoming Webhook 또는 Slack Bot API 중 하나를 선택해 `windforce.release.published`를 사람이 읽을 수 있는 메시지로 변환한다. Telegram connector는 같은 event fixture와 signature verifier를 재사용한다. Control Plane public URL이 connector에 설정된 경우에만 event subject를 상세 화면 링크로 변환한다.
...
Slack provider API smoke, provider credential 저장, 메시지 template과 process restart를 견디는 durable deduplication은 connector repository의 배포 완료 조건이다.
...
| 10 | Messenger connector integration | public fixtures, verifier, generic receiver, contract tests |
...
- generic receiver로 release-to-delivery 경로를 검증하고, messenger provider smoke는 별도 connector repository에서 검증한다.
