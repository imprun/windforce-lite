	...
	paths[fmt.Sprintf("/api/w/%s/jobs/{id}/result", workspaceID)] = map[string]any{
		"get": map[string]any{
			...
			"responses": map[string]any{
				"200": oapiResponse("Finished run — status is \"success\", \"failure\", or \"canceled\"; result holds the output, or the failure detail when failed.", map[string]any{
					"type": "object",
					"properties": map[string]any{
						"status": oapiStatusSchema(),
...
func oapiStatusSchema() map[string]any {
	return map[string]any{
		...
		"description": "Terminal run status. A failed action surfaces as failure within a 200 response, not as an HTTP error.",
	}
}
