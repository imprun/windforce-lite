...
import { type Client, type InputConfig } from "../lib/api";
...
export function ClientDetailPage({
  ...
  if (state.loading && !state.data) return <Layout title="Client Registry"><Loading /></Layout>;
  if (state.error || !state.data) {
    return <Layout title="Client not found"><ErrorNotice message={state.error || "Client not found."} onRetry={state.reload} /></Layout>;
  }
  ...
  return (
    <Layout
      ...
      actions={
        <>
          <Link className="button" to="/clients">Back to registry</Link>
          <button className="button" type="button" onClick={() => setEditingClient(true)}>Edit client</button>
        </>
      }
    >
      <nav className="tabBar" aria-label="Client detail tabs">
        {tabs.map((item) => (
          <Link
            ...
            to={item.key === "overview" ? `/clients/${client.id}` : `/clients/${client.id}/${item.key}`}
          >
            {item.label}
          </Link>
        ))}
      ...
      {activeTab === "input-settings" ? (
        <ClientInputSettings client={client} configs={configs} apps={apps} selectedAppKey={appKey} onChanged={state.reload} />
      ) : null}
      ...
      {editingClient ? (
        <ClientDialog
          ...
          onSaved={() => { setEditingClient(false); state.reload(); }}
...
function ClientOverview({ client, configs }: { client: Client; configs: InputConfig[] }) {
  ...
  const latest = useMemo(
    () => configs.reduce((current, config) => !current || Date.parse(config.updated_at) > Date.parse(current.updated_at) ? config : current, undefined as typeof configs[number] | undefined),
    [configs],
  );
  return (
    <>
      ...
      <Panel title="Configuration summary" subtitle="Current client-specific coverage across released apps.">
        {configs.length ? (
          <DefinitionList items={[
            ...
            ["Last settings change", latest ? `${formatRelative(latest.updated_at)} · ${latest.updated_by}` : "—"],
          ]} />
...
function ClientAudit({ clientID }: { clientID: string }) {
  ...
  return (
    <Panel title="Audit trail" subtitle="Registry and input-setting changes for this client.">
      ...
      {state.data ? <AuditEventTable events={state.data} emptyTitle="No changes have been recorded for this client." /> : null}
    </Panel>
  );
}
